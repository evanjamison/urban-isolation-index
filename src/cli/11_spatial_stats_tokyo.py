...
Compute spatial statistics for Tokyo's Designed Isolation Index (D-IRI):
...
Usage (from project root):
  ...
  .\.venv\Scripts\python.exe -m src.cli.11_spatial_stats_tokyo `
...
def load_and_merge(index_path: str, wards_geojson: str) -> gpd.GeoDataFrame:
    ...
    # Your GeoJSON uses N03_007 as the ward code
    ...
    if gdf.empty:
        raise ValueError("Merge produced no rows – check ward codes.")
...
def save_results(gdf: gpd.GeoDataFrame, out_dir: str):
    """Save LISA table and cluster map."""
    os.makedirs(out_dir, exist_ok=True)

    # 1) Save table
    out_csv = os.path.join(out_dir, "tokyo_diri_lisa_results.csv")
    cols_to_save = [
        "ward_jis",
        "ward_name",
        "iri_designed",
    ...
    # 2) Save hotspot map
    out_png = os.path.join(out_dir, "tokyo_diri_lisa_clusters.png")
...
def main():
    p = argparse.ArgumentParser(
        description="Spatial statistics for Tokyo D-IRI (Moran's I + LISA)."
    )
    p.add_argument(
        "--index-path",
        required=True,
    ...
    p.add_argument(
        "--wards-geojson",
        required=True,
        help="GeoJSON file for Tokyo wards (e.g., data/external/jp_tokyo_wards.geojson).",
    )
    p.add_argument(
        "--out-dir",
        required=True,
    ...
    print("\n🎉 Spatial statistics pipeline completed.")
